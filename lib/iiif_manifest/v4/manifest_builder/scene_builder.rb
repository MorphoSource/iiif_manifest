module IIIFManifest
  module V4
    class ManifestBuilder
      class SceneBuilder
        attr_reader :record, :parent, :iiif_scene_factory, :content_builder,
                    :choice_builder, :iiif_annotation_page_factory

        def initialize(record,
                       parent,
                       iiif_scene_factory:,
                       content_builder:,
                       choice_builder:,
                       iiif_annotation_page_factory:)
          @record = record
          @parent = parent
          @iiif_scene_factory = iiif_scene_factory
          @content_builder = content_builder
          @choice_builder = choice_builder
          @iiif_annotation_page_factory = iiif_annotation_page_factory
          apply_record_properties
          # Presentation 2.x approach
          attach_image if display_image
          # Presentation 3.0 approach
          attach_content if display_content
        end

        def scene
          @scene ||= iiif_scene_factory.new
        end

        def path
          path = "#{parent.manifest_url}/scene/#{record.id}"
          path << "##{record.media_fragment}" if record.respond_to?(:media_fragment)
          path
        end

        def apply(items)
          return items if scene.items.blank?
          items << scene
        end

        private

          def display_image
            record.display_image if ( record.respond_to?(:display_image) && record.display_image.present? )
          end

          def display_content
            Array.wrap(record.display_content) if ( record.respond_to?(:display_content) && record.display_content.present? )
          end

          def apply_record_properties
            scene['id'] = path
            scene.label = ManifestBuilder.language_map(record.to_s) if record.to_s.present?
            annotation_page['id'] = "#{path}/annotation_page/#{annotation_page.index}"
            scene.items = [annotation_page]
          end

          def annotation_page
            @annotation_page ||= iiif_annotation_page_factory.new
          end

          def attach_image
            content_builder.new(display_image).apply(scene)
          end

          def attach_mesh
            content_builder.new(display_mesh).apply(scene)
          end

          def attach_volume
            content_builder.new(display_volume).apply(scene)
          end

          def attach_content
            if display_content.size == 1
              content_builder.new(display_content.first).apply(scene)
            else
              choice_builder.new(display_content).apply(scene)
            end
          end
      end
    end
  end
end
